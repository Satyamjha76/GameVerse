...
# Check if 'hidden' column exists in 'evidence'
cursor.execute("PRAGMA table_info(evidence)")
columns = [col[1] for col in cursor.fetchall()]
if "hidden" not in columns:
    print("Updating database schema: Adding 'hidden' column to 'evidence' table.")

    # Rename old table
    cursor.execute("ALTER TABLE evidence RENAME TO old_evidence")

    # Create new evidence table with 'hidden' column
    cursor.execute("""
    CREATE TABLE evidence (
        ...
        clue TEXT,
        type TEXT,
        hidden INTEGER DEFAULT 0,  -- New column (0 = visible, 1 = hidden)
    ...
    # Migrate old data (all previous clues will be marked as 'hidden=0' by default)
    cursor.execute("""
    INSERT INTO evidence (id, suspect_id, weapon_id, location_id, clue, type, hidden)
    SELECT id, suspect_id, weapon_id, location_id, clue, type, 0 FROM old_evidence
    """)
...
# Generate multiple pieces of evidence (some are hidden)
true_clues = [
...
# Generate additional hidden clues (more detailed but not immediately visible)
hidden_clues = [
    f"A fingerprint of {culprit} was found on the {murder_weapon}.",
    f"Security footage shows {culprit} entering the {crime_location} at the estimated time of murder.",
    f"A witness saw {culprit} hiding something suspicious in the {crime_location}."
]
...
for clue in all_clues:
    ...
    cursor.execute("""
    INSERT INTO evidence (suspect_id, weapon_id, location_id, clue, type, hidden)
    VALUES (
        (SELECT id FROM suspects WHERE name = ?),
        (SELECT id FROM weapons WHERE name = ?),
        (SELECT id FROM locations WHERE name = ?),
        ?, ?, 0  -- Normal clue (not hidden)
        )
...
for clue in hidden_clues:
    ...
    VALUES (
        ...
        (SELECT id FROM weapons WHERE name = ?),
        (SELECT id FROM locations WHERE name = ?),
        ?, 'true', 1  -- Mark as hidden
        )
    """, (culprit, murder_weapon, crime_location, clue))
...
class CustomMessagebox(tk.Toplevel):
    def __init__(self, title, message, color):
        ...
        self.bg_image = Image.open("C:/Users/jhasa/OneDrive/Desktop/GameVerse/GameVerse/assets/murderMysteryassets/bg.jpeg")
...
class CustomSimpleDialog(tk.Toplevel):
    def __init__(self, title, prompt):
        ...
        self.bg_image = Image.open("mm.png")
...
def execute_query():
    ...
    try:
        ...
        # Check if the player is trying to unlock hidden clues
        if "hidden=1" in query.lower():
            show_message("Secret Clue Discovered!", "You've unlocked hidden evidence!", "#F1C40F")
...
def solve_mystery():
    ...
    if (suspect_guess and weapon_guess and location_guess and
        ...
        attempts_left -= 1
        if attempts_left > 0:
            show_message("Wrong Guess",
                         f"Incorrect guess. Attempts left: {attempts_left}. Maybe there's more evidence you haven't found yet...",
                         "#E74C3C")
        else:
            reset_evidence()
            show_message("Game Over",
                         "All attempts are over! The evidence has changed.\nHint: Not all evidence is visible. Try using SQL to find hidden information!",
                         "#E74C3C")
...
bg_image = Image.open("C:/Users/jhasa/OneDrive/Desktop/GameVerse/GameVerse/assets/murderMysteryassets/bg.jpeg")
...
title_label = tk.Label(root, text="SQL Murder Mystery LEVEL 3", font=("Helvetica", 18, "bold"), fg="white",
                       bg="#23272A")
...
back_button = tk.Button(button_frame, text="BACK", command=lambda: back("home_mm"), font=("Arial", 14, "bold"),
                        bg="#99AAB5", fg="black")
